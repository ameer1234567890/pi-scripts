...
# *-* coding: utf-8 -*-
"""Log garden statitics to ThingSpeak"""
...
def heat_index(temperature, humidity):
    ...
    p3 = c7*temp**2*rel_humidity + c8*temp*rel_humidity**2 + \
        c9*temp**2*rel_humidity**2
    heat_index = p1 + p2 + p3
...
if humidity is not None and temperature is not None:
    ...
    key = Config.CHANNEL_KEY  # noqa: F821
    channel_url = 'https://api.thingspeak.com/update?api_key=' + \
        key + '&field1=' + str(round(temperature, 2)) + \
        '&field2=' + str(round(humidity, 2)) + '&field3=' + \
        str(round(dewpoint, 2)) + '&field4=' + str(round(heatindex, 2)) + \
        '&field5=' + str(light_level)
